...
''' This programme plots a connectivity matrix for the output from a particle tracking model (PTM).

In addition to plotting a representation of how release sites are connected, this programme
outputs a MATLAB file with the matrix used to produce the plot and a summary of the input parameters.
...
def count_for_location_in_polygon(loc):
    ''' Tallies particle counts, in a specified polygon, for a given location, for all release
    days and months, and for all time steps. The decision to do this per location was chosen,
    because this is likely to be the largest dimension and is therefore the most efficient
    division for multiprocessing.
    '''
    particle_counts_loc_in_polygon = 0

    for d1 in range(np.shape(lats_loop)[1]):
        for d2 in range(np.shape(lats_loop)[2]):
            for d3 in range(np.shape(lats_loop)[3]):
                point = Point(lons_loop[loc, d1, d2, d3], lats_loop[loc, d1, d2, d3])
                if polygon_j.contains(point):
                    particle_counts_loc_in_polygon += 1

    return particle_counts_loc_in_polygon
...
output_dir = '../output/' + datetime.now().strftime('%Y-%m-%dT%H%M') + '_plot_connectivity/'
...
connectivity_mat = np.zeros((ns, ns)) # Matrix to count connectivity counts
...
for i in range(ns):
    ...
    # Iterate through other sites to count particles from the release site
    particles_in_polygons_count = np.zeros(ns)
    for j in range(ns):
        polygon_j = Polygon(zip(site_lons[j,:], site_lats[j,:]))
        pool = mp.Pool(num_cores)
        particle_counts_catch_in_polygon = pool.map(count_for_location_in_polygon, range(np.shape(lats_loop)[0]))
        particles_in_polygons_count[j] = np.sum(particle_counts_catch_in_polygon)
        pool.close()

    # Add particle count to connectivity_mat
    connectivity_mat[i,:] = particles_in_polygons_count

    # Save date to .mat file in folder for this site, if keep_polygon_mats=1
    if keep_polygon_mats == 1:
        dict_data = {
            'particles_polygon_counts': particles_in_polygons_count,
        }

        scipy.io.savemat(output_dir + 'site_' + str(site_num) + '/connectivity_for_site_' + str(site_num) + '.mat', dict_data)
...
plt.savefig(output_dir + 'all_sites_connectivity.png', dpi=dpi)
...
scipy.io.savemat(output_dir + '/data_connectivity.mat', dict_data_all)
